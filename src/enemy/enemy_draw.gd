...
func _build_body():
	...
	match enemy_type:
		...
		_: _build_humanoid(skin_tex, is_old_format)
...
func _build_humanoid(skin_tex, is_old_format: bool):
	...
	left_arm_pivot.position = Vector2(0, -2)
	...
	right_arm_pivot.position = Vector2(0, -2)
	...
	right_arm.position = Vector2(0, 2)
	right_arm.z_index = 2
	...
	head_pivot.position = Vector2(0, -2)
...
func _build_creeper(skin_tex):
	# Chibi creeper: 2x4 body + legs, 8x8 head, 4 short legs
	var leg_region = Rect2(0, 20, 2, 4)
	
	# 1. Back Left Leg (far side)
	back_left_leg_pivot = Node2D.new()
	back_left_leg_pivot.position = Vector2(0, 2)
	...
	back_right_leg_pivot.position = Vector2(0, 2)
